...
export default{
    ...
    methods: {
        async getReplies(){
            let result = await axios.get('/replies/' + this.thread.id)
        ...
        async getModeratedThreads(){
            let result = await axios.get('/moderator/' + this.user.id)
            this.moderatorOfThreads = result.data
        },
        async getAllThreads(user){
            let result = await axios.get('/threads')
            result = result.data
            console.log(result)
            if (user.username !== ''){
        ...
        async changeModerator(thread){
            await axios.get('/addModerator/' + this.user.id + '/' + thread.id)
        },
        async removeUser(){
            await axios.delete('/deleteUser/' + this.user.id)
        },
        async lockThread(){
            await axios.get('/lockThread/' + this.thread.id)
        },
        async removeThread(reply){
            await axios.get('/deleteReply/' + reply.id)
        }
    },
    created(){
