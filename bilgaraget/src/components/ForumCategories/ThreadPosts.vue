...
export default{
    ...
    methods: {
        async getReplies(){
            let result = await axios.get('/rest/replies/' + this.thread.id)
         ...
         async getUser(){
            ...
            let result = await axios.get('/user/' + this.thread.creator)
        ...
        async getModeratedThreads(){
            let result = await axios.get('/rest/moderator/' + this.user.id)
            this.moderatorOfThreads = result.data
        },
        async getAllThreads(user){
            let result = await axios.get('/rest/threads')
            result = result.data
            console.log(result)
            if (user.username !== ''){
        ...
        async changeModerator(thread){
            await axios.get('/rest/addModerator/' + this.user.id + '/' + thread.id)
        },
        async removeUser(){
            await axios.delete('/deleteUser/' + this.user.id)
        },
        async lockThread(){
            await axios.get('/rest/lockThread/' + this.thread.id)
        },
        async removeThread(reply){
            await axios.get('/rest/deleteReply/' + reply.id)
        }
    },
    created(){
